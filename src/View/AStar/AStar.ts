...
class AStar {
	...
	public constructor(mapArr: Grid[][]) {
		...
		for (var i: number = 0; i < mapArr.length; i++) {
			this._nodeArr[i] = [];
			for (var j: number = 0; j < mapArr.length; j++) {
	...
	public find(startX: number, startY: number, endX: number, endY: number): Array<AStarNode> {
		var c: number = GameMainManager.instance.mapC;//地图最大行
		var r: number = GameMainManager.instance.mapR;//地图最大列
	...
	private search(): boolean {
		...
		var c: number = GameMainManager.instance.mapC;
		var r: number = GameMainManager.instance.mapR;
